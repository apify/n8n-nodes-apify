...
import { APIFY_CONSOLE_BASE_URL } from '../../helpers/consts';
...
const resourceLocatorProperty: INodeProperties = {
	...
	modes: [
		...
		},
		{
			...
			url: `=${APIFY_CONSOLE_BASE_URL}/storage/key-value-stores/{{ $value }}`,
		},
	],
};
...
export async function listKeyValueStores(
	...
	const searchResults = await apiRequestAllItems.call(this, '/v2/key-value-stores', {
		method: 'GET',
		qs: {
			limit: 100,
			offset: 0,
	...
	return {
		results: items.map((b: any) => ({
			name: b.name,
			value: b.id,
			url: `${APIFY_CONSOLE_BASE_URL}/storage/key-value-stores/${b.id}`,
			description: b.name,
		})),
	};
}
