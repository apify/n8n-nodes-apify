...
- The optional cost cap `maxTotalChargeUsd` ("Maximum Cost per Run (USD)") is hand-declared in the `properties.ts` of all four run operations (`actors/run-actor`, `actors/run-actor-and-get-dataset`, `actor-tasks/run-task`, `actor-tasks/run-task-and-get-dataset`) — add it to every one or the spec in `__tests__/Apify.node.spec.ts` fails. Default must stay `null` (= no limit); the query param is only sent when the value is non-null **and** `> 0`, so `0`/empty also means no limit. That guard is duplicated in `resources/executeActor.ts` and in both actor-task `execute.ts` files, since the task handlers build their own `qs` instead of going through `executeActor`.
