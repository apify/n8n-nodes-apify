...
- Resource locators (`resources/*ResourceLocator.ts`) build Apify Console links in three places per file — the `url` mode's validation/`extractValue` regexes, the `id` mode's `url` template, and the `list` method's per-item `url`. Keep all three on the same canonical route, or list results link somewhere the URL mode won't accept: runs → `https://console.apify.com/actors/runs/<runId>` (fixed in `a64d6af`), Actors → `/actors/<actorId>/input`, tasks → `/actors/tasks/<taskId>/input`, key-value stores → `/storage/key-value-stores/<storeId>`.
